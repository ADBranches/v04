...
/**
 * IMPORTANT:
 *  - All services must call api/apiService/apiMethods with RELATIVE paths ONLY.
 *    e.g. "/admin/users", "/destinations"
 *  - Do NOT pass absolute URLs or duplicate the base URL.
 */
...
api.interceptors.request.use(
  (config) => {
    ...
    // Ensure headers exists before mutation
...
api.interceptors.response.use(
  (response) => {
    ...
    // Keep default axios response shape, but ensure data is returned directly by helpers
    return response;
  },
  (error) => {
    ...
    if (import.meta.env.MODE === 'development') {
      ...
      if (status === 404) {
        console.info(
          `ℹ️ API 404 [${error.config?.method?.toUpperCase()} ${error.config?.url}]`,
          response?.data
        );
      } else {
        console.groupCollapsed(
          `❌ API Error [${status ?? 'NO_RESPONSE'}] ${error.config?.method?.toUpperCase()} ${error.config?.url}`
        );
  ...
  async delete<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
    try {
      const response = await api.delete(endpoint, {
        headers: options.headers,
        // If you need query params for DELETE:
        // params: options.params,
      });
...
function enhanceError(error: any, method: string, url: string): ApiError {
  ...
  if (response?.status === 404) {
    enhancedError.code = enhancedError.code || 'NOT_FOUND';
    if (!enhancedError.message || enhancedError.message === error.message) {
      enhancedError.message = 'Resource not found';
    }
  } else if (response?.status === 400) {
    enhancedError.code = enhancedError.code || 'BAD_REQUEST';
  } else if (response?.status === 401) {
    enhancedError.code = enhancedError.code || 'UNAUTHORIZED';
  } else if (response?.status === 403) {
    enhancedError.code = enhancedError.code || 'FORBIDDEN';
  } else if (response?.status === 409) {
    enhancedError.code = enhancedError.code || 'CONFLICT';
  } else if (response?.status >= 500) {
    enhancedError.code = enhancedError.code || 'SERVER_ERROR';
  } else if (!response) {
    enhancedError.code = enhancedError.code || 'NETWORK_ERROR';
    if (!enhancedError.message) {
      enhancedError.message = 'Network error';
    }
  }
  ...
  if (import.meta.env.MODE === 'development') {
    ...
    // Expected "not found" in app flows should not look like crashes
    if (status === 404) {
      console.info(
        `ℹ️ API 404 [${error.config?.method?.toUpperCase()} ${error.config?.url}]`,
        response?.data
...
function showNotification(
  ...
  notification.innerHTML = `
    <div class="flex items-center justify-between">
      ...
      <button class="ml-4 text-current hover:opacity-70 text-lg font-bold" aria-label="Close notification">
        ×
      </button>
    </div>
  `;
...
export const apiUtils = {
  ...
  // Get API base URL (normalized, without trailing slash)
  getBaseURL(): string {
    return BASE_URL;
  },
