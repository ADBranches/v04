...
export default async function runPrismaSeed() {
  try {
    ...
    try {
      ...
      if (err.code === 'P2003') {
        console.error(
          '💡 Hint: Foreign key constraint violation — check dependent ModerationLog or Booking entries before deleting Destination.'
        );
      }
    ...
    const destinations = [
      {
        ...
        description:
          "Home to nearly half of the world's mountain gorillas, offering an unforgettable trekking experience in Uganda's lush rainforest.",
      ...
      },
      {
        ...
        description:
          'Experience the dramatic Murchison Falls, where the Nile River squeezes through a narrow gorge, and enjoy game drives to spot lions, elephants, and giraffes.',
      ...
      },
      {
        ...
        description:
          'A biodiversity hotspot with tree-climbing lions, hippos, and a variety of bird species, perfect for safaris and boat cruises.',
      ...
      },
      {
        name: 'Lake Bunyonyi',
        description:
          'The second deepest lake in Africa, known for its stunning scenery and 29 islands. Perfect for relaxation and water activities.',
    ...
    const bwindi = await prisma.destination.findFirst({
      where: { name: 'Bwindi Impenetrable Forest' },
    });
    const murchison = await prisma.destination.findFirst({
      where: { name: 'Murchison Falls National Park' },
    });
    const queenElizabeth = await prisma.destination.findFirst({
      where: { name: 'Queen Elizabeth National Park' },
    });
    const bunyonyi = await prisma.destination.findFirst({
    ...
    for (const log of moderationLogs) {
      try {
        ...
        if (log.content_type === 'destination') {
          ...
          if (!destExists) {
            console.warn(
              `⚠️ Skipping destination moderation log — no Destination found with id=${log.content_id}`
            );
            continue;
          }
        }
        ...
        console.log(
          `✅ Moderation log created for ${log.content_type} #${
            log.content_id || 'N/A'
          } (ID ${created.id})`
        );
      } catch (err) {
        console.error(
          `❌ Failed moderation log for ${log.content_type} #${
            log.content_id || 'N/A'
    ...
    for (const booking of bookings) {
      ...
      console.log(
        `✅ Booking created for ${booking.number_of_people} people to destination ${booking.destination_id}`
      );
    }
...
// Optional: allow direct `node prisma/seed.js`
if (import.meta.url === `file://${process.argv[1]}`) {
  runPrismaSeed()
    .then(() => {
      console.log('✅ Prisma seed completed (direct)');
      process.exit(0);
    })
    .catch((err) => {
